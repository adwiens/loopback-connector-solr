...
var Query = /** @class */ (function () {
    ...
    Query.replaceOrCreateCb = function (error, success, callback, doc) {
        if (error)
            callback(error, undefined);
        else {
    ...
    Query.existsCb = function (error, success, callback) {
        if (error)
            callback(error, undefined);
        if (success)
            callback(undefined, { exists: success.response.numFound > 0 });
    };
    return Query;
}());
        ...
        if (!doc["id"]) {
            ...
            client.get('query', query, function (err, suc) {
                ...
                if (suc && suc.response.numFound === 0)
                    client.add([doc], function (err, suc) { return Query.replaceOrCreateCb(err, suc, cb, doc); });
                else
                    throw new Error("UUID collision check failed");
            });
        }
